...
fn crt_to_pmr(
    ...
    let mut gadget = |state: &mut ArithBuilderState, x: &ArithNode<Feed>, y: &ArithNode<Feed>| -> Result<Vec<ArithNode<Feed>>, ArithCircuitError> {

        let p = x.modulus();
        let q = y.modulus();
        let x_ = mod_change(state,x, p + q - 1)?;
        let y_ = mod_change(state, y, p + q - 1)?;
        let z = sub(state, &x_, &y_)?;

        // self.proj(&z, q, Some(gadget_projection_tt(p, q)))

        z
            .iter()
            .map(|e| state.add_proj_gate(e, q, Some(gadget_projection_tt(p, q)).unwrap()))
    ...
    for i in 1..=n {
        for j in i + 1..=n {
            let z = gadget(state,x[i - 1][i].as_ref().unwrap(), x[i - 1][j].as_ref().unwrap())?;
            x[i][j] = Some(z);
        }
    }
